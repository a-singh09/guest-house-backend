...
router.get("/:id/bookingHistory/:type", async (req,res) => {
 ...
 try {
    ...
    if(type === 'upcoming') {
        ...
        const upcomingBookings = await Booking.find({"roomBooker.email": email, status: { $not: { $in: ['rejected', 'autoReject', 'cancelled'] }}, startDate: {$gte: currentDate } });
...
router.delete("/approved/rejected/:id", async (req,res) => {
    const id = req.params.id;
    try {
        await RejectedUser.deleteOne({_id: id});
        res.json(`rejected user with id ${id} deleted successfully.`)
    } catch (error) {
        res.json({registered_user_error: error.message})
    }
})
